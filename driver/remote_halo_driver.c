...
int invoke_pack_or_unpack(Chunk *chunk, Settings *settings, int face, int depth, int offset, bool pack, double *buffer);
...
void remote_halo_driver(Chunk *chunks, Settings *settings, int depth) {
  ...
  for (int cc = 0; cc < settings->num_chunks_per_rank; ++cc) {
    if (chunks[cc].neighbours[CHUNK_LEFT] != EXTERNAL_FACE) {
      ...
      send_recv_message(settings, chunks[cc].left_send, chunks[cc].left_recv, buffer_len, chunks[cc].neighbours[CHUNK_LEFT], 0, 1,
    ...
    if (chunks[cc].neighbours[CHUNK_RIGHT] != EXTERNAL_FACE) {
      ...
      send_recv_message(settings, chunks[cc].right_send, chunks[cc].right_recv, buffer_len, chunks[cc].neighbours[CHUNK_RIGHT], 1, 0,
                        &(requests[num_messages]), &(requests[num_messages + 1]));
  ...
  for (int cc = 0; cc < settings->num_chunks_per_rank; ++cc) {
    if (chunks[cc].neighbours[CHUNK_BOTTOM] != EXTERNAL_FACE) {
      ...
      send_recv_message(settings, chunks[cc].bottom_send, chunks[cc].bottom_recv, buffer_len, chunks[cc].neighbours[CHUNK_BOTTOM], 0, 1,
                        &(requests[num_messages]), &(requests[num_messages + 1]));
    ...
    if (chunks[cc].neighbours[CHUNK_TOP] != EXTERNAL_FACE) {
      ...
      send_recv_message(settings, chunks[cc].top_send, chunks[cc].top_recv, buffer_len, chunks[cc].neighbours[CHUNK_TOP], 1, 0,
                        &(requests[num_messages]), &(requests[num_messages + 1]));
