...
void solve(Chunk *chunks, Settings *settings, int tt, double *wallclock_prev) {
  ...
  switch (settings->solver) {
    case JACOBI_SOLVER: jacobi_driver(chunks, settings, rx, ry, &error); break;
    case CG_SOLVER: cg_driver(chunks, settings, rx, ry, &error); break;
    case CHEBY_SOLVER: cheby_driver(chunks, settings, rx, ry, &error); break;
    case PPCG_SOLVER: ppcg_driver(chunks, settings, rx, ry, &error); break;
  }
