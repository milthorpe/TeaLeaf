...
void run_set_chunk_data(Chunk *chunk, Settings *settings) {
  ...
  set_chunk_data(settings, chunk->x, chunk->y, chunk->left, chunk->bottom, chunk->cell_x, chunk->cell_y, chunk->vertex_x, chunk->vertex_y,
                 chunk->volume, chunk->x_area, chunk->y_area);
}

void run_set_chunk_state(Chunk *chunk, Settings *settings, State *states) {
  set_chunk_state(chunk->x, chunk->y, chunk->vertex_x, chunk->vertex_y, chunk->cell_x, chunk->cell_y, chunk->density, chunk->energy0,
...
void run_kernel_initialise(Chunk *chunk, Settings *settings) {
  kernel_initialise(settings, chunk->x, chunk->y, &(chunk->density0), &(chunk->density), &(chunk->energy0), &(chunk->energy), &(chunk->u),
                    ...
                    &(chunk->ext->comm_buffer), &(chunk->cg_alphas), &(chunk->cg_betas), &(chunk->cheby_alphas), &(chunk->cheby_betas));
}

void run_kernel_finalise(Chunk *chunk, Settings *settings) {
  kernel_finalise(chunk->density0, chunk->density, chunk->energy0, chunk->energy, chunk->u, chunk->u0, chunk->p, chunk->r, chunk->mi,
                  chunk->w, chunk->kx, chunk->ky, chunk->sd, chunk->volume, chunk->x_area, chunk->y_area, chunk->cell_x, chunk->cell_y,
                  chunk->cell_dx, chunk->cell_dy, chunk->vertex_dx, chunk->vertex_dy, chunk->vertex_x, chunk->vertex_y,
                  chunk->ext->comm_buffer, chunk->cg_alphas, chunk->cg_betas, chunk->cheby_alphas, chunk->cheby_betas);
}
...
void run_pack_or_unpack(Chunk *chunk, Settings *settings, int depth, int face, bool pack, double *field, double *buffer) {
  START_PROFILING(settings->kernel_profile);
  ...
  pack_or_unpack(chunk->x, chunk->y, depth, settings->halo_depth, face, pack, field, chunk->ext->comm_buffer);
