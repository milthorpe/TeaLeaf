...
#pragma acc kernels loop independent collapse(2) present(u[ : xy], p[ : xy])
...
#pragma acc kernels loop independent collapse(2) present(w[ : xy], r[ : xy], kx[ : xy], ky[ : xy], r[ : xy], p[ : xy], u0[ : xy], u[ : xy])
  for (int jj = halo_depth; jj < y - halo_depth; ++jj) {
    for (int kk = halo_depth; kk < x - halo_depth; ++kk) {
      const int index = kk + jj * x;
