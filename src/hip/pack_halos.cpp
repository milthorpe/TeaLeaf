...
void pack_or_unpack(Chunk *chunk, Settings *settings, int depth, int face, bool pack, double *field, double *buffer) {
  ...
  if (!pack) {
    hipMemcpy(chunk->ext->d_comm_buffer, buffer, buffer_length * sizeof(double), hipMemcpyHostToDevice);
  ...
  if (pack) {
    hipMemcpy(buffer, chunk->ext->d_comm_buffer, buffer_length * sizeof(double), hipMemcpyDeviceToHost);
    check_errors(__LINE__, __FILE__);
  }
}
