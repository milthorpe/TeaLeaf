...
typedef void (*pack_kernel_f)(const int x, const int y, const int halo_depth, double *field, double *buffer, const int depth);
...
__global__ void unpack_left(const int x, const int y, const int halo_depth, double *field, double *buffer, const int depth);
...
void pack_or_unpack(Chunk *chunk, Settings *settings, int depth, int face, bool pack, double *field, double *buffer) {
  ...
  if (!pack) {
    cudaMemcpy(chunk->ext->d_comm_buffer, buffer, buffer_length * sizeof(double), cudaMemcpyHostToDevice);
    check_errors(__LINE__, __FILE__);
  }

  int num_blocks = ceil(buffer_length / (double)BLOCK_SIZE);

  kernel<<<num_blocks, BLOCK_SIZE>>>(chunk->x, chunk->y, settings->halo_depth, field, chunk->ext->d_comm_buffer, depth);

  if (pack) {
    cudaMemcpy(buffer, chunk->ext->d_comm_buffer, buffer_length * sizeof(double), cudaMemcpyDeviceToHost);
    check_errors(__LINE__, __FILE__);
  }
}
