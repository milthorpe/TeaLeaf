...
void run_kernel_initialise(Chunk *chunk, Settings *settings) {
  kernel_initialise(settings, chunk->x, chunk->y, &(chunk->density0), &(chunk->density), &(chunk->energy0), &(chunk->energy), &(chunk->u),
                    ...
                    &(chunk->ext->comms_buffer), &(chunk->ext->host_comms_mirror), &(chunk->cg_alphas), &(chunk->cg_betas),
                    &(chunk->cheby_alphas), &(chunk->cheby_betas));
}

void run_kernel_finalise(Chunk *chunk, Settings *settings) {
  kernel_finalise(chunk->cg_alphas, chunk->cg_betas, chunk->cheby_alphas, chunk->cheby_betas);
}
...
#define UPDATE_FACE(face, buffer, update_kernel)                                                                                           \
  if (chunk->neighbours[face] == EXTERNAL_FACE) {                                                                                          \
    START_PROFILING(settings->kernel_profile);                                                                                             \
    update_kernel(chunk->x, chunk->y, settings->halo_depth, buffer, face, depth);                                                          \
...
void run_pack_or_unpack(Chunk *chunk, Settings *settings, int depth, int face, bool pack, FieldBufferType field, double *buffer) {
  START_PROFILING(settings->kernel_profile);

  const int buffer_length = (face == CHUNK_LEFT || face == CHUNK_RIGHT) ? chunk->y * depth : chunk->x * depth;

  if (!pack) {
    KokkosHelper::PackMirror<double>(*chunk->ext->host_comms_mirror, buffer, buffer_length);
...
#define PACK_HALO(face_val, pack_val, pack_kernel)                                                                                         \
  if (face == face_val && pack == pack_val) {                                                                                              \
    pack_kernel(chunk->x, chunk->y, settings->halo_depth, *chunk->ext->comms_buffer, *field, depth);                                       \
  }
  ...
  if (pack) {
    ...
    KokkosHelper::UnpackMirror<double>(buffer, *chunk->ext->host_comms_mirror, buffer_length);
  }
...
void run_jacobi_init(Chunk *chunk, Settings *settings, double rx, double ry) {
  ...
  jacobi_init(chunk->x, chunk->y, settings->halo_depth, settings->coefficient, rx, ry, *chunk->u, *chunk->u0, *chunk->density,
              *chunk->energy, *chunk->kx, *chunk->ky);
