...
void set_chunk_state(int x, int y, double *vertex_x, double *vertex_y, double *cell_x, double *cell_y, double *density, double *energy0,
                     double *u, const int num_states, State *states) {
    ...
    for (int jj = 0; jj < y; ++jj) {
      for (int kk = 0; kk < x; ++kk) {
        ...
        if (states[ss].geometry == RECTANGULAR) {
          apply_state = (vertex_x[kk + 1] >= states[ss].x_min && vertex_x[kk] < states[ss].x_max && vertex_y[jj + 1] >= states[ss].y_min &&
                         vertex_y[jj] < states[ss].y_max);
        } else if (states[ss].geometry == CIRCULAR) {
          double radius = sqrt((cell_x[kk] - states[ss].x_min) * (cell_x[kk] - states[ss].x_min) +
                               (cell_y[jj] - states[ss].y_min) * (cell_y[jj] - states[ss].y_min));

          apply_state = (radius <= states[ss].radius);
        } else if (states[ss].geometry == POINT) {
          apply_state = (vertex_x[kk] == states[ss].x_min && vertex_y[jj] == states[ss].y_min);
        }
